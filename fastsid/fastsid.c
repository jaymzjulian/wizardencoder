...
WORD wavetable00[2];
WORD wavetable10[4096];
WORD wavetable20[4096];
WORD wavetable30[4096];
WORD wavetable40[8192];
WORD wavetable50[8192];
WORD wavetable60[8192];
WORD wavetable70[8192];

#endif

/* Noise tables */
#define NOISETABLESIZE 256
BYTE noiseMSB[NOISETABLESIZE];
BYTE noiseMID[NOISETABLESIZE];
BYTE noiseLSB[NOISETABLESIZE];
...
DWORD sidreadclocks[9];

vreal_t lowPassParam[0x800];
vreal_t bandPassParam[0x800];
vreal_t filterResTable[16];
const float filterRefFreq = 44100.0;
signed char ampMod1x8[256];

inline void dofilter(voice_t *pVoice)
{
    if (!pVoice->filter)
        return;
...
void set_adsr(voice_t *pv, BYTE fm)
{
    int i;
...
#if 0
void print_voice(char *buf, voice_t *pv)
{
    const char *m = "ADSRI";
#ifdef WAVETABLES
...
char *fastsid_dump_state(sound_t *psid)
{
    ...
    sprintf(buf, "#SID: clk=%ld v=%d s3=%d\n",
            psid->maincpu_clk, psid->vol, psid->has3);
...
inline void setup_sid(sound_t *psid)
{
    if (!psid->update)
        return;
...
inline void setup_voice(voice_t *pv)
{
    if (!pv->update)
        return;
...
SWORD fastsid_calculate_single_sample(sound_t *psid, int i)
{
    DWORD o0, o1, o2;
    int dosync1, dosync2;
    voice_t *v0, *v1, *v2;
...
void fastsid_store(sound_t *psid, WORD addr, BYTE byte)
{
    ...
    psid->laststore = byte;
    psid->laststorebit = 8;
    psid->laststoreclk = psid->maincpu_clk;
}
